...
Write-Host "  EchoBot - Iniciando servicos...       " -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""

# 1. Backend (FastAPI via uvicorn do venv)
Write-Host "[1/3] Backend (FastAPI)..." -ForegroundColor Green
Start-Process powershell -ArgumentList "-NoExit", "-Command", "Write-Host 'BACKEND' -ForegroundColor Cyan; Set-Location '$root\backend'; .\venv\Scripts\uvicorn.exe app.main:app --reload" -WindowStyle Normal

Start-Sleep -Milliseconds 500

# 2. Voice Bridge (Node.js)
Write-Host "[2/3] Voice Bridge (Node.js)..." -ForegroundColor Green
Start-Process powershell -ArgumentList "-NoExit", "-Command", "Write-Host 'VOICE BRIDGE' -ForegroundColor Magenta; Set-Location '$root\voice-bridge'; npm start" -WindowStyle Normal
